      ...
      <Container className="relative grid gap-12 lg:grid-cols-2 lg:items-center">
        <Reveal direction="left" className="relative">
          ...
          <div className="absolute -right-4 -bottom-6 hidden rounded-2xl border border-border bg-surface p-5 shadow-xl sm:block">
            <p className="font-display text-sm font-bold text-gradient">
              {dict.about.badgeCardTitle}
            </p>
            <p className="text-sm text-muted">{dict.about.badgeCardSub}</p>
          </div>
          <div className="absolute -top-5 -left-5 hidden animate-float items-center gap-2 rounded-2xl border border-border bg-surface px-4 py-3 shadow-xl sm:flex">
            ...
            <div>
              <p className="text-xs font-semibold text-foreground">{dict.about.chipTitle}</p>
              <p className="text-[10px] text-muted">{dict.about.chipSub}</p>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal direction="right" delay={0.1}>
          ...
          <h2 className="mt-4 font-display text-3xl font-semibold tracking-tight text-foreground sm:text-4xl lg:text-5xl">
            {dict.about.titleStart} {dict.about.titleHighlight}
          </h2>
          ...
          <div className="mt-8 flex items-center gap-3 rounded-2xl border border-dashed border-border bg-surface/70 p-5">
            ...
            <p className="text-sm leading-relaxed text-muted">{dict.about.footerText}</p>
          </div>
        </Reveal>
      </Container>
    </Section>
